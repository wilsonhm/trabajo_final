...
import 'package:servicio_asistencia/ui/estudiante/evento/evento_edit.dart';
import 'package:servicio_asistencia/ui/estudiante/evento/evento_form.dart';
...
class MainEventoE extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(create: (_) => EventoBloc(EventoRepository())),
      ],
      child: MaterialApp(
...
class _EventoUIState extends State< EventoUI> {
  ...
  void initState() {
    super.initState();
    BlocProvider.of< EventoBloc>(context).add(ListarEventoEvent());
  }
      ...
      home: Scaffold(
        ...
        body: BlocBuilder< EventoBloc,  EventoState>(
          builder: (context, state) {
            if (state is  EventoLoadedState) {
              return _buildListView(state.eventoList);
            } else {
              return Center(
  ...
  Widget _buildListView(List< EventoModelo> eventos) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 8.0, horizontal: 16.0),
      child: ListView.builder(
        itemBuilder: (context, index) {
           EventoModelo evento = eventos[index];
          return Padding(
                        ...
                        children: <Widget>[
                            ...
                            onPressed: () {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) =>EventoFormEdit(modelR: evento),
                                ),
                              ).then(onGoBack);
                            },
                          ),
                            ...
                            onPressed: () {
                              showDialog(
                                ...
                                if (value.toString() == "Success") {
                                  BlocProvider.of< EventoBloc>(context).add(
                                    DeleteEventoEvent(evento),
                                  );
                                }
                              });
                            },
